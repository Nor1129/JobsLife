...
public final class Jobslevelpl extends JavaPlugin implements Listener {
    ...
    public boolean connect() {
        try {
            ...
            getLogger().info("MySQL 연결 성공: " + database);
            return true;

        } catch (SQLException e) {
            getLogger().severe("MySQL 연결 실패: " + e.getMessage());
            e.printStackTrace();
            connection = null;
            return false;
        }
    ...
    void savePlacedBlock(Location loc, Material type) {
        if (!isConnected() || loc == null || loc.getWorld() == null) return;

        placedBlocks.add(loc);

        String sql = """
        INSERT INTO placed_blocks (world, x, y, z, type) 
        VALUES (?, ?, ?, ?, ?) 
        ON DUPLICATE KEY UPDATE type = VALUES(type), placed_at = CURRENT_TIMESTAMP
        """;
    ...
    private void updateScoreboard(Player player) {
        ...
        switch (worldName) {
            ...
            case "deepseaworld":
                ...
                break;
            case "lobby":
                translatedWorldName = "로비";
                break;
            case "blackmarket":
                translatedWorldName = "암시장";
                break;
            default:
